...
function saveUser() {
    ...
    if (user.value.user_name?.trim() && user.value.user_email?.trim()) {
        const token = getTokenFromCookie();

        // If user_id exists, update (PUT), else create (POST)
        ...
        <Dialog v-model:visible="userDialog" :style="{ width: '450px' }" header="User Details" :modal="true">
            <div class="flex flex-col gap-6">
                ...
                <div>
                    ...
                    <InputText id="user_email" v-model="user.user_email" required="true" rows="3" cols="20" fluid />
                </div>
                <div>
